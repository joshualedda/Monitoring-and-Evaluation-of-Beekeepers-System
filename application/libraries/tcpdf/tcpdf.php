...
class TCPDF {
	...
	public function Footer() {
		...
		if ($this->getRTL()) {
			$this->SetX($this->original_rMargin);
			//Change the height from 0 to 10 because the page was too near the line
			$this->Cell(0, 10, $pagenumtxt, 'T', 0, 'L');
		} else {
			$this->SetX($this->original_lMargin);
			//Add this line for having the date on the footer
			$this->Cell(0, 10, date('Y-m-d'), 'T', 0, 'L');
