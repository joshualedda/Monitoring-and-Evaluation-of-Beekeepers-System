<div id="main">
  <div class="main-container">
    ...
    <div class="page-header-card">
      <div>
        <h4><i class="ph ph-flag me-2 text-primary"></i><?php echo $this->lang->line('Phase'); ?></h4>
        <p>Manage different phases of beekeeping operations.</p>
      </div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 small">
          <li class="breadcrumb-item"><a href="<?php echo base_url('dashboard') ?>" class="text-decoration-none">Home</a></li>
          <li class="breadcrumb-item active"><?php echo $this->lang->line('Phase'); ?></li>
        </ol>
      </nav>
    </div>
      ...
      <div class="col-md-12 col-xs-12">
        ...
        <div class="dt-card">
          <div class="dt-card-header">
            <h5 class="mb-0">All Phases</h5>
            <div class="d-flex gap-2">
              <?php if(in_array('viewPhase', $user_permission)): ?>
                <a href="<?php echo base_url('report06/report06/phase'); ?>" target="_blank" class="btn btn-light border d-flex align-items-center">
              ...
              <?php if(in_array('createPhase', $user_permission)): ?>
                <button class="btn-dt-add" data-bs-toggle="modal" data-bs-target="#addModal" onclick="createFunc()">
                  <i class="ph ph-plus mb-1"></i> <?php echo $this->lang->line('Add Phase'); ?>
                </button>
              <?php endif; ?>
            </div>
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Add Phase'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('phase/create') ?>" method="post" id="createForm">
        <div class="modal-body px-4 py-4">
          <div class="mb-4">
            <label for="phase_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control form-control-lg rounded-3 shadow-sm border-light-subtle" id="phase_name" name="phase_name" autocomplete="off" autofocus placeholder="Enter phase name">
          </div>
...
<?php if(in_array('updatePhase', $user_permission)): ?>
<!-- edit modal -->
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Edit Phase'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('phase/update') ?>" method="post" id="updateForm">
        <div class="modal-body px-4 py-4">
          <div id="messages_edit"></div>

          <div class="mb-4">
            <label for="edit_phase_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control form-control-lg rounded-3 shadow-sm border-light-subtle" id="edit_phase_name" name="edit_phase_name" autocomplete="off" placeholder="Enter phase name">
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-danger d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Delete Phase'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
...
{ 
  $.ajax({
    url: base_url + 'phase/fetchPhaseDataById/'+id,
    type: 'post',
    dataType: 'json',
    success:function(response) {
      $("#updateForm")[0].reset();
      $(".text-danger").remove();
      $("#edit_phase_name").val(response.name);
