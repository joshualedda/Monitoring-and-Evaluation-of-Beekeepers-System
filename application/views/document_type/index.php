<div id="main">
  <div class="main-container">
    ...
    <div class="page-header-card">
      <div>
        <h4><i class="ph ph-file-text me-2 text-primary"></i><?php echo $this->lang->line('Document Type'); ?></h4>
        <p>Manage various document classifications and attachment types.</p>
      </div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 small">
          ...
          <li class="breadcrumb-item active small text-primary fw-medium"><?php echo $this->lang->line('Document Type'); ?></li>
        </ol>
      </nav>
    </div>
      ...
      <div class="col-md-12 col-xs-12">
        ...
        <div class="dt-card">
          <div class="dt-card-header">
            <h5 class="mb-0">All Document Types</h5>
            <div class="d-flex gap-2">
              <?php if(in_array('viewDocumentType', $user_permission)): ?>
                <a href="<?php echo base_url('report06/report06/document_type'); ?>" target="_blank" class="btn btn-light border d-flex align-items-center" title="Print List">
              ...
              <?php if(in_array('createDocumentType', $user_permission)): ?>
                <button class="btn-dt-add" data-bs-toggle="modal" data-bs-target="#addModal" onclick="createFunc()">
                   <i class="ph ph-plus mb-1"></i> <?php echo $this->lang->line('Add Document Type'); ?>
                </button>
              <?php endif; ?>
            </div>
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Add Document Type'); ?>
        </h4>
      ...
      <form role="form" action="<?php echo base_url('document_type/create') ?>" method="post" id="createForm">
        <div class="modal-body px-4 py-4 text-start">
          <div class="mb-4">
            <label for="document_type_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="document_type_name" name="document_type_name" autocomplete="off" autofocus placeholder="Enter document type name">
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Edit Document Type'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      ...
      <form role="form" action="<?php echo base_url('document_type/update') ?>" method="post" id="updateForm">
        <div class="modal-body px-4 py-4 text-start">
          ...
          <div class="mb-4">
            <label for="edit_document_type_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="edit_document_type_name" name="edit_document_type_name" autocomplete="off" placeholder="Enter document type name">
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-danger d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Delete Document Type'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
...
$(document).ready(function() {
  ...
  manageTable = $('#manageTable').DataTable({
    'ajax': base_url + 'document_type/fetchDocumentTypeData',
...
{ 
  $.ajax({
    url: base_url + 'document_type/fetchDocumentTypeDataById/'+id,
    type: 'post',
    dataType: 'json',
    success:function(response) {
      $("#updateForm")[0].reset();
