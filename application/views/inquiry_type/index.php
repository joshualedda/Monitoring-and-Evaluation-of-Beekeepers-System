<div id="main">
  <div class="main-container">
    ...
    <div class="page-header-card">
      <div>
        <h4><i class="ph ph-question me-2 text-primary"></i><?php echo $this->lang->line('Inquiry Type'); ?></h4>
        <p>Manage different types of inquiries and their classifications.</p>
      </div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 small">
          <li class="breadcrumb-item"><a href="<?php echo base_url('dashboard') ?>" class="text-decoration-none">Home</a></li>
          <li class="breadcrumb-item active"><?php echo $this->lang->line('Inquiry Type'); ?></li>
        </ol>
      </nav>
    </div>
      ...
      <div class="col-md-12 col-xs-12">
        ...
        <div class="dt-card">
          <div class="dt-card-header">
            <h5 class="mb-0">All Inquiry Types</h5>
            <div class="d-flex gap-2">
              <?php if(in_array('viewInquiryType', $user_permission)): ?>
                <a href="<?php echo base_url('report06/report06/inquiry_type'); ?>" target="_blank" class="btn btn-light border d-flex align-items-center">
                  <i class="ph ph-printer me-2 fs-5"></i> Print
                </a>
              <?php endif; ?>
              <?php if(in_array('createInquiryType', $user_permission)): ?>
                <button class="btn-dt-add" data-bs-toggle="modal" data-bs-target="#addModal" onclick="createFunc()">
                  <i class="ph ph-plus mb-1"></i> <?php echo $this->lang->line('Add Inquiry Type'); ?>
                </button>
              <?php endif; ?>
            </div>
          </div>
            ...
            <div class="datatable-wrapper">
              <table id="manageTable" class="table align-middle mb-0">
                ...
                <tr>                
                  ...
                  <?php if(in_array('updateInquiryType', $user_permission) || in_array('deleteInquiryType', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Add Inquiry Type'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('inquiry_type/create') ?>" method="post" id="createForm">
        <div class="modal-body px-4 py-4">
          <div class="mb-3">
            <label for="inquiry_type_code" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Code'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3" id="inquiry_type_code" name="inquiry_type_code" maxlength="10" autocomplete="off" placeholder="Enter code">
          </div>

          <div class="mb-4">
            <label for="inquiry_type_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3" id="inquiry_type_name" name="inquiry_type_name" autocomplete="off" placeholder="Enter inquiry type name">
          </div>
...
<?php if(in_array('updateInquiryType', $user_permission)): ?>
<!-- edit modal -->
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Edit Inquiry Type'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('inquiry_type/update') ?>" method="post" id="updateForm">
        <div class="modal-body px-4 py-4">
          <div id="messages_edit"></div>

          <div class="mb-3">
            <label for="edit_inquiry_type_code" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Code'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3" id="edit_inquiry_type_code" name="edit_inquiry_type_code" maxlength="10" autocomplete="off" placeholder="Enter code">
          </div>

          <div class="mb-4">
            <label for="edit_inquiry_type_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3" id="edit_inquiry_type_name" name="edit_inquiry_type_name" autocomplete="off" placeholder="Enter inquiry type name">
          </div>
...
<?php if(in_array('deleteInquiryType', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-danger d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Delete Inquiry Type'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('inquiry_type/remove') ?>" method="post" id="removeForm">
        <div class="modal-body px-4 py-4">
...
$(document).ready(function() {
  $("#inquiry_typeNav").addClass('active');

  // initialize the datatable 
  manageTable = $('#manageTable').DataTable({
    'ajax': base_url + 'inquiry_type/fetchInquiryTypeData',
        ...
        if(response.success === true) {
          ...
          if(response.messages instanceof Object) {
            $.each(response.messages, function(index, value) {
              var id = $("#"+index);
              id.closest('.mb-3, .mb-4').find('.text-danger').remove();
              id.after(value);
            });
          } else {
...
{ 
  $.ajax({
    url: base_url + 'inquiry_type/fetchInquiryTypeDataById/'+id,
    type: 'post',
    dataType: 'json',
    success:function(response) {
      $("#updateForm")[0].reset();
      $(".text-danger").remove();
      $("#edit_inquiry_type_code").val(response.code);
      $("#edit_inquiry_type_name").val(response.name);
