<div id="main">
  <div class="main-container">
    ...
    <div class="page-header-card">
      <div>
        <h4><i class="ph ph-house-line me-2 text-primary"></i><?php echo $this->lang->line('Barangay'); ?></h4>
        <p>Manage barangays and their respective Local Government Units.</p>
      </div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 small">
          ...
          <li class="breadcrumb-item active small text-primary fw-medium"><?php echo $this->lang->line('Barangay'); ?></li>
        </ol>
      </nav>
    </div>
      ...
      <div class="col-md-12 col-xs-12">
        ...
        <div class="dt-card">
          <div class="dt-card-header">
            <h5 class="mb-0">All Barangays</h5>
            <div class="d-flex gap-2">
              <?php if(in_array('viewBarangay', $user_permission)): ?>
                <a href="<?php echo base_url('report06/report06/barangay') ?>" target="_blank" class="btn btn-light border d-flex align-items-center" title="Print List">
                   <i class="ph ph-printer me-2 fs-5"></i> Print
                </a>
              <?php endif; ?>
              <?php if(in_array('createBarangay', $user_permission)): ?>
                <button class="btn-dt-add" data-bs-toggle="modal" data-bs-target="#addModal" onclick="createFunc()">
                   <i class="ph ph-plus mb-1"></i> <?php echo $this->lang->line('Add Barangay'); ?>
                </button>
              <?php endif; ?>
            </div>
          </div>
            ...
            <div class="datatable-wrapper">
              <table id="manageTable" class="table align-middle mb-0">
                ...
                <tr>               
                  ...
                  <th><?php echo $this->lang->line('Lgu');?></th> 
                  <th><?php echo $this->lang->line('Code'); ?></th>
                  <th width="15%"><?php echo $this->lang->line('Active'); ?></th>
                  <?php if(in_array('updateBarangay', $user_permission) || in_array('deleteBarangay', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Add Barangay'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('barangay/create') ?>" method="post" id="createForm">
        <div class="modal-body px-4 py-4">
          <div class="mb-3">
              <label class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Lgu'); ?><font color="red"> *</font></label>
              <select name="lgu" id="lgu" class="form-control select2" style="width: 100%;">
                <option value="" hidden selected>Select LGU</option>
              </select>
          </div>

          <div class="row g-3 mb-3">
             <div class="col-md-6 text-start">
               <label for="barangay_code" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Code'); ?></label>
               <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="barangay_code" name="barangay_code" autocomplete="off" placeholder="Enter code">
             </div>
          ...
          <div class="mb-0">
            <label for="barangay_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="barangay_name" name="barangay_name" autocomplete="off" placeholder="Enter barangay name">
          </div>         
        </div>
...
<?php if(in_array('updateBarangay', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Edit Barangay'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('barangay/update') ?>" method="post" id="updateForm">
        <div class="modal-body px-4 py-4">
          <div id="messages_edit"></div>
          <div class="mb-3">
              <label class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Lgu'); ?><font color="red"> *</font></label>
              <select name="edit_barangay_lgu" id="edit_lgu" class="form-control select2" style="width: 100%;">
                <option value="" hidden selected>Select LGU</option>
              </select>
          </div>
          ...
          <div class="row g-3 mb-3">
             <div class="col-md-6">
                <label for="edit_barangay_code" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Code'); ?></label>
                <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="edit_barangay_code" name="edit_barangay_code" autocomplete="off" placeholder="Enter code">
              </div>
          ...
          <div class="mb-0">
            <label for="edit_barangay_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="edit_barangay_name" name="edit_barangay_name" autocomplete="off" placeholder="Enter barangay name">
          </div>  
        </div>
...
<?php if(in_array('deleteBarangay', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-danger d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Delete Barangay'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('barangay/remove') ?>" method="post" id="removeForm">
        <div class="modal-body px-4 py-4">
...
$(document).ready(function() {
  ...
  //---> creation of the drop-down list LGU
    var fetchLgus = function(target) {
        var $select = $(target);
        $.ajax({
            url: base_url+'lgu/fetchActiveLgu',
            dataType: "JSON", 
            success: function (data) {
                //iterate over the data and append a select option
                $.each(data, function (key, val) {
                    $select.append('<option value="' + val.id + '">' + val.name +', '+val.province_name+ '</option>');  
                }); 
            }, 
            error: function () {
  ...
  $("#barangayNav").addClass('active');

  // initialize the datatable 
  manageTable = $('#manageTable').DataTable({
    'ajax': base_url + 'barangay/fetchBarangayData',
...
{ 
  $.ajax({
    url: base_url + 'barangay/fetchBarangayDataById/'+id,
    type: 'post',
    dataType: 'json',
    success:function(response) {
      $("#updateForm")[0].reset();
      ...
      $('#edit_lgu').val(response.lgu_id).trigger('change');
      $("#edit_barangay_code").val(response.code);
      $("#edit_barangay_name").val(response.name);
          ...
          success:function(response) {
            ...
            if(response.success === true) {
              ...
              if(response.messages instanceof Object) {
                $.each(response.messages, function(index, value) {
                  var id = $("#"+index);
                  if(index == 'edit_barangay_lgu') id = $("#edit_lgu");
                  id.closest('.mb-3, .form-group').find('.text-danger').remove();
                  id.after(value);
                });
              } else {
