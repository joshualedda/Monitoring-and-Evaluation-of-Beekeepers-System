<div id="main">
  <div class="main-container">
    ...
    <div class="page-header-card">
      <div>
        <h4><i class="ph ph-package me-2 text-primary"></i><?php echo $this->lang->line('Product'); ?></h4>
        <p>Manage various beekeeping products and inventory items.</p>
      </div>
          ...
          <div class="dt-card-header">
            ...
            <div class="d-flex gap-2">
              <?php if(in_array('viewProduct', $user_permission)): ?>
                <a href="<?php echo base_url('report06/report06/product'); ?>" target="_blank" class="btn btn-light border d-flex align-items-center">
              ...
              <?php if(in_array('createProduct', $user_permission)): ?>
                <button class="btn-dt-add" data-bs-toggle="modal" data-bs-target="#addModal" onclick="createFunc()">
                  <i class="ph ph-plus mb-1"></i> <?php echo $this->lang->line('Add Product'); ?>
                </button>
              <?php endif; ?>
            </div>
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Add Product'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('product/create') ?>" method="post" id="createForm">
        <div class="modal-body px-4 py-4">
          <div class="mb-4">
            <label for="product_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control form-control-lg rounded-3 shadow-sm border-light-subtle" id="product_name" name="product_name" autocomplete="off" autofocus placeholder="Enter product name">
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Edit Product'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('product/update') ?>" method="post" id="updateForm">
        <div class="modal-body px-4 py-4">
          <div id="messages_edit"></div>

          <div class="mb-4">
            <label for="edit_product_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control form-control-lg rounded-3 shadow-sm border-light-subtle" id="edit_product_name" name="edit_product_name" autocomplete="off" placeholder="Enter product name">
          </div>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-danger d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Delete Product'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
...
$(document).ready(function() {
  ...
  manageTable = $('#manageTable').DataTable({
    'ajax': base_url + 'product/fetchProductData',
...
{ 
  $.ajax({
    url: base_url + 'product/fetchProductDataById/'+id,
    type: 'post',
    dataType: 'json',
    success:function(response) {
      $("#updateForm")[0].reset();
