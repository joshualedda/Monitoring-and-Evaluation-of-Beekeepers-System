<div id="main">
  <div class="main-container">
    ...
    <div class="page-header-card">
      <div>
        <h4><i class="ph ph-bug me-2 text-primary"></i><?php echo $this->lang->line('Species'); ?></h4>
        <p>Manage the different species of bees kept in apiaries.</p>
      </div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 small">
          <li class="breadcrumb-item"><a href="<?php echo base_url('dashboard') ?>" class="text-decoration-none">Home</a></li>
          <li class="breadcrumb-item active"><?php echo $this->lang->line('Species'); ?></li>
        </ol>
      </nav>
    </div>
      ...
      <div class="col-md-12 col-xs-12">
        ...
        <div class="dt-card">
          <div class="dt-card-header">
            <h5 class="mb-0">All Bee Species</h5>
            <div class="d-flex gap-2">
              <?php if(in_array('viewSpecies', $user_permission)): ?>
                <a href="<?php echo base_url('report06/report06/species'); ?>" target="_blank" class="btn btn-light border d-flex align-items-center">
                  <i class="ph ph-printer me-2 fs-5"></i> Print
                </a>
              <?php endif; ?>
              <?php if(in_array('createSpecies', $user_permission)): ?>
                <button class="btn-dt-add" data-bs-toggle="modal" data-bs-target="#addModal" onclick="createFunc()">
                  <i class="ph ph-plus mb-1"></i> <?php echo $this->lang->line('Add Species'); ?>
                </button>
              <?php endif; ?>
            </div>
          </div>
            ...
            <div class="datatable-wrapper">
              <table id="manageTable" class="table align-middle mb-0">
                ...
                <tr>                
                  ...
                  <?php if(in_array('updateSpecies', $user_permission) || in_array('deleteSpecies', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Add Species'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('species/create') ?>" method="post" id="createForm">
        <div class="modal-body px-4 py-4">
          <div class="mb-4">
            <label for="species_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control form-control-lg rounded-3 shadow-sm border-light-subtle" id="species_name" name="species_name" autocomplete="off" autofocus placeholder="Enter species name">
          </div>
...
<?php if(in_array('updateSpecies', $user_permission)): ?>
<!-- edit modal -->
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Edit Species'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('species/update') ?>" method="post" id="updateForm">
        <div class="modal-body px-4 py-4">
          <div id="messages_edit"></div>

          <div class="mb-4">
            <label for="edit_species_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control form-control-lg rounded-3 shadow-sm border-light-subtle" id="edit_species_name" name="edit_species_name" autocomplete="off" placeholder="Enter species name">
          </div>
...
<?php if(in_array('deleteSpecies', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-danger d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Delete Species'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('species/remove') ?>" method="post" id="removeForm">
        <div class="modal-body px-4 py-4">
...
$(document).ready(function() {
  $("#speciesNav").addClass('active');

  // initialize the datatable 
  manageTable = $('#manageTable').DataTable({
    'ajax': base_url + 'species/fetchSpeciesData',
...
{ 
  $.ajax({
    url: base_url + 'species/fetchSpeciesDataById/'+id,
    type: 'post',
    dataType: 'json',
    success:function(response) {
      $("#updateForm")[0].reset();
      $(".text-danger").remove();
      $("#edit_species_name").val(response.name);
