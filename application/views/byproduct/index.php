<div id="main">
  <div class="main-container">
    ...
    <div class="page-header-card">
      <div>
        <h4><i class="ph ph-nut me-2 text-primary"></i><?php echo $this->lang->line('By-product'); ?></h4>
        <p>Manage various hive by-products and their active status.</p>
      </div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 small">
          ...
          <li class="breadcrumb-item active small text-primary fw-medium"><?php echo $this->lang->line('By-product'); ?></li>
        </ol>
      </nav>
    </div>
      ...
      <div class="col-md-12 col-xs-12">
        ...
        <div class="dt-card">
          <div class="dt-card-header">
            <h5 class="mb-0">All By-products</h5>
            <div class="d-flex gap-2">
              <?php if(in_array('viewByproduct', $user_permission)): ?>
                <a href="<?php echo base_url('report06/report06/byproduct'); ?>" target="_blank" class="btn btn-light border d-flex align-items-center" title="Print List">
                   <i class="ph ph-printer me-2 fs-5"></i> Print
                </a>
              <?php endif; ?>
              <?php if(in_array('createByproduct', $user_permission)): ?>
                <button class="btn-dt-add" data-bs-toggle="modal" data-bs-target="#addModal" onclick="createFunc()">
                   <i class="ph ph-plus mb-1"></i> <?php echo $this->lang->line('Add By-product'); ?>
                </button>
              <?php endif; ?>
            </div>
          </div>
            ...
            <div class="datatable-wrapper">
              <table id="manageTable" class="table align-middle mb-0">
                ...
                <tr>                
                  ...
                  <?php if(in_array('updateByproduct', $user_permission) || in_array('deleteByproduct', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Add By-product'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('byproduct/create') ?>" method="post" id="createForm">
        <div class="modal-body px-4 py-4 text-start">
          <div class="mb-4">
            <label for="byproduct_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="byproduct_name" name="byproduct_name" autocomplete="off" autofocus placeholder="Enter byproduct name">
          </div>
...
<?php if(in_array('updateByproduct', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-dark l-spacing-1 d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Edit By-product'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('byproduct/update') ?>" method="post" id="updateForm">
        <div class="modal-body px-4 py-4 text-start">
          <div id="messages_edit"></div>

          <div class="mb-4">
            <label for="edit_byproduct_name" class="form-label fw-semibold text-secondary small text-uppercase l-spacing-1"><?php echo $this->lang->line('Name'); ?><font color="red"> *</font></label>
            <input type="text" class="form-control rounded-3 shadow-sm border-light-subtle" id="edit_byproduct_name" name="edit_byproduct_name" autocomplete="off" placeholder="Enter byproduct name">
          </div>
...
<?php if(in_array('deleteByproduct', $user_permission)): ?>
    ...
    <div class="modal-content border-0 shadow rounded-4">
      <div class="modal-header border-bottom-0 pb-0 pt-4 px-4">
        <h4 class="modal-title fw-bold text-danger d-flex align-items-center">
          ...
          <?php echo $this->lang->line('Delete By-product'); ?>
        </h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form role="form" action="<?php echo base_url('byproduct/remove') ?>" method="post" id="removeForm">
...
$(document).ready(function() {
  ...
  manageTable = $('#manageTable').DataTable({
    'ajax': base_url + 'byproduct/fetchByproductData',
...
{ 
  $.ajax({
    url: base_url + 'byproduct/fetchByproductDataById/'+id,
    type: 'post',
    dataType: 'json',
    success:function(response) {
      $("#updateForm")[0].reset();
      $(".text-danger").remove();
      $("#edit_byproduct_name").val(response.name);
