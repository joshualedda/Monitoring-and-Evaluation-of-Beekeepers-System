...
class Topography extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = $this->lang->line('Topography');

	}


	//--> Redirects to the manage topography page

	public function index()
	{

		if(!in_array('viewTopography', $this->permission)) {
			redirect('dashboard', 'refresh');
		}

		$this->render_template('topography/index', $this->data);	
	}	

	
	//--> It checks if it gets the topography id and retreives
	//    the topography information from the topography model and 
	//    returns the data into json format. 
	//    This function is invoked from the view page.

	public function fetchTopographyDataById($id) 
	{
		if($id) {
			$data = $this->model_topography->getTopographyData($id);
	...
	//--> Fetches the topography value from the topography table 
	//    This function is called from the datatable ajax function
	
	public function fetchTopographyData()
	{
		$result = array('data' => array());

		$data = $this->model_topography->getTopographyData();

		foreach ($data as $key => $value) {

			$buttons = '';

			if(in_array('updateTopography', $this->permission)) {
			...
			if(in_array('deleteTopography', $this->permission)) {
	...
	//--> It checks the topography form validation 
	//    and if the validation is true (valid) then it inserts the data into the database 
	//    and returns the json format operation messages
	
	public function create()
	{
		if(!in_array('createTopography', $this->permission)) {
			redirect('dashboard', 'refresh');
		}

		$response = array();

		$this->form_validation->set_rules('topography_name', $this->lang->line('Name'), 'trim|required');
		$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

        if ($this->form_validation->run() == TRUE) {
        	$data = array(
        		'name' => $this->input->post('topography_name'),
        		'active' => $this->input->post('active'),	
        	);

        	$create = $this->model_topography->create($data);
        	if($create == true) {
	...
	public function update($id)
	{
		...
		if($id) {
			$this->form_validation->set_rules('edit_topography_name', $this->lang->line('Name'), 'trim|required');
			$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
	        		'name' => $this->input->post('edit_topography_name'),
	        		'active' => $this->input->post('edit_active'),	
	        	);

	        	$update = $this->model_topography->update($data, $id);
	        	if($update == true) {
	        		$response['success'] = true;
	...
	//--> It removes the topography information from the database 
	//    and returns the json format operation messages
	...
	public function remove()
	{
		...
		$topography_id = $this->input->post('topography_id');
		...
		if($topography_id) {
			...
			$total_beekeeper = $this->model_topography->checkIntegrity($topography_id);
			//---> If no colony have this information, we can delete
			//$total_beekeeper=0;
            if ($total_beekeeper == 0) {        
				$delete = $this->model_topography->remove($topography_id);
				if($delete == true) {
					$response['success'] = true;
