...
class profile extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = $this->lang->line('Profile');
	...
	public function fetchProfileData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('updateProfile', $this->permission)) {
                $buttons .= '<a href="'.base_url('profile/update/'.$value['id']).'" class="btn-dt btn-dt-edit" title="Edit"><i class="ph ph-pencil-simple"></i></a>';
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
        		$msg_error = $this->lang->line('Successfully created'); 
	...
	//--> It removes the information from the database 
	//    and it sends the operation message into the session flashdata and display on the manage profile page
	...
	public function remove()
	{
		...
		if($profile_id) {
            ...
            if ($total_user == 0) {        
				...
				$response['success'] = false;
				$response['messages'] = $this->lang->line('At least one user uses this information.  You cannot delete.');}
