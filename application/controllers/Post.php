...
{   
    public function __construct()
    {
        ...
        $this->data['active_tab'] = $this->input->get('tab') ?? 'post';
    }
   ...
   //    and it sends the operation message into the session flashdata and display on the manage post page
	...
	public function create()
	{
        ...
        $this->form_validation->set_rules('post_category', $this->lang->line('Category'), 'trim|required');
        $this->form_validation->set_rules('post_title', $this->lang->line('Title'), 'trim|required');
		$this->form_validation->set_rules('post_text', $this->lang->line('Text'), 'trim|required');
		$this->form_validation->set_rules('date_from', $this->lang->line('Date from'), 'trim|required');
		$this->form_validation->set_rules('date_to', $this->lang->line('Date to'), 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ( ! $this->upload->do_upload('post_image')) {
                ...
                else {$msg_error = $this->lang->line('This type of document is not allowed or the document is too large.'); 
	...
	{      
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if($update == true) {
                ...
                redirect('post/update/'.$post_id."?tab=post", 'refresh');
            } else {
                $msg_error = $this->lang->line('Error occurred'); 
                $this->session->set_flashdata('error', $msg_error);
                redirect('post/update/'.$post_id, 'refresh');}
       ...
       $response = array();
        ...
        if($post_id) {           
            ...
            if ($total_document == 0) {  
                ...
                $response['success'] = false;
                $response['messages'] = $this->lang->line('You must delete the documents attached before deleting the post.');}
    ...
    public function uploadDocument()
    {
        ...
        if ( ! $this->upload->do_upload('post_document')){
            ...
            $this->session->set_flashdata('warning', $msg_error);
            redirect('post/update/'.$this->session->post_id."?tab=document", 'refresh');
            }
        else
            {
