<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Barangay extends Admin_Controller 
{
	public function __construct()
	{
		parent::__construct();

		$this->not_logged_in();

		$this->data['page_title'] = $this->lang->line('Barangay');

	}


    //--> Redirects to the manage barangay page

	public function index()
	{
		if(!in_array('viewBarangay', $this->permission)) {
			redirect('dashboard', 'refresh');
		}
		$this->render_template('barangay/index', $this->data);	
	}


	//--> It retrieve the specific barangay information via a barangay id
	//    and returns the data in json format

	public function fetchBarangayDataById($id) 
	{
		if($id) {
			$data = $this->model_barangay->getBarangayData($id);
			echo json_encode($data);
		}
	}

	public function fetchBarangayDataByMunicipality() 
	{
			if($this->input->post('municipality_id'))
			{
				echo $this->model_barangay->getBarangayDataByMunicipalityId($this->input->post('municipality_id'));
			}
	}


		public function fetchActiveBarangay() 
	{
		$data = $this->model_barangay->getActiveBarangay();
		echo json_encode($data);

	}

	
	//--> It retrieves all the barangay data from the database 
	//    This function is called from the datatable ajax function
	//    The data is return based on the json format.
	
	public function fetchBarangayData()
	{
		$result = array('data' => array());

		$data = $this->model_barangay->getBarangayData();

		foreach ($data as $key => $value) {

			$lgu_data = $this->model_lgu->getLguData($value['lgu_id']);


			$buttons = '';

			if(in_array('updateBarangay', $this->permission)) {
				$buttons = '<button type="button" class="btn-dt btn-dt-edit" title="Edit" onclick="editFunc('.$value['id'].')" data-bs-toggle="modal" data-bs-target="#editModal"><i class="ph ph-pencil-simple"></i></button>';
			 	$name='  <a data-bs-target="#editModal" onclick="editFunc('.$value['id'].')" data-bs-toggle="modal" href="#editModal" class="text-decoration-none fw-medium">'.$value['name'].'</a>';

			}

			if(in_array('deleteBarangay', $this->permission)) {
				$buttons .= ' <button type="button" class="btn-dt btn-dt-delete" title="Delete" onclick="removeFunc('.$value['id'].')" data-bs-toggle="modal" data-bs-target="#removeModal"><i class="ph ph-trash"></i></button>';}

			$active = ($value['active'] == 1) ? '<span class="dt-badge dt-badge-success">'.$this->lang->line('Active').'</span>' : '<span class="dt-badge dt-badge-danger">'.$this->lang->line('Inactive').'</span>';

			$result['data'][$key] = array(			
				$name,
				$lgu_data['name'],
				$value['code'],
				$active,
				$buttons
			);
		} // /foreach

		echo json_encode($result);
	}


	//--> If the validation is not true (not valid), then it provides the validation error on the json format
    //    If the validation for each input is valid then it creates the data into the database and 
    //    returns an appropriate message in the json format.

	public function create()
	{
		if(!in_array('createBarangay', $this->permission)) {
			redirect('dashboard', 'refresh');
		}

		$response = array();
		$this->form_validation->set_rules('lgu', $this->lang->line('Lgu'), 'required');
		$this->form_validation->set_rules('barangay_name', $this->lang->line('Name'), 'trim|required');
		$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

        if ($this->form_validation->run() == TRUE) {
        	$data = array(
        		'lgu_id' =>$this->input->post('lgu'),
        		'code' => $this->input->post('barangay_code'),
        		'name' => $this->input->post('barangay_name'),
        		'active' => $this->input->post('active'),	
        	);

        	$create = $this->model_barangay->create($data);
        	if($create == true) {
        		$response['success'] = true;
        		$response['messages'] = $this->lang->line('Successfully created');
        
        	}
        	else {
        		$response['success'] = false;
        		$response['messages'] = $this->lang->line('Error in the database while creating the information');			
        	}
        }
        else {
        	$response['success'] = false;
        	foreach ($_POST as $key => $value) {
        		$response['messages'][$key] = form_error($key);
        	}
        }

        echo json_encode($response);
	}	

	//--> If the validation is not true (not valid), then it provides the validation error on the json format
    //    If the validation for each input is valid then it updates the data into the database and 
    //    returns an appropriate message in the json format.

	public function update($id)
	{
		if(!in_array('updateBarangay', $this->permission)) {
			redirect('dashboard', 'refresh');
		}

		$response = array();

		if($id) {
			$this->form_validation->set_rules('edit_barangay_lgu', $this->lang->line('Lgu'), 'trim|required');
			$this->form_validation->set_rules('edit_barangay_name', $this->lang->line('Name'), 'trim|required');
			$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
	        		'lgu_id'=> $this->input->post('edit_barangay_lgu'),
	        		'code' => $this->input->post('edit_barangay_code'),
	        		'name' => $this->input->post('edit_barangay_name'),
	        		'active' => $this->input->post('edit_active'),	
	        	);

	        	$update = $this->model_barangay->update($data, $id);
	        	if($update == true) {
	        		$response['success'] = true;
	        		$response['messages'] = $this->lang->line('Successfully updated');
	        		}
	        	else {
	        		$response['success'] = false;
	        		$response['messages'] = $this->lang->line('Error in the database while updating the information');			
	        		}
	        	}
	        else {
	        	$response['success'] = false;
	        	foreach ($_POST as $key => $value) {
	        		$response['messages'][$key] = form_error($key);
	        	}
	        }
		}
		else {
			$response['success'] = false;
    		$response['messages'] = $this->lang->line('Error please refresh the page again');
		}

		echo json_encode($response);
	}

	
	//--> If checks if the barangay id is provided on the function, if not then an appropriate message 
	//    is return on the json format
    //    If the validation is true (valid) then it removes the data into the database and returns an appropriate 
    //    message in the json format.
    
	public function remove()
	{
		if(!in_array('deleteBarangay', $this->permission)) {
			redirect('dashboard', 'refresh');
		}
		
		$barangay_id = $this->input->post('barangay_id');

		$response = '';
		$response = array();

		if($barangay_id) {
			//---> Validate if the information is used in beekeeper/colony table
			$total_rows = $this->model_barangay->checkIntegrity($barangay_id);
			//$total_rows=0;
			//---> If no beekeeper/colony have this information, we can delete
            if ($total_rows == 0) {        
				$delete = $this->model_barangay->remove($barangay_id);
				if($delete == true) {
					$response['success'] = true;
					$response['messages'] = $this->lang->line('Successfully deleted');}
				else {
					$response['success'] = false;
					$response['messages'] = $this->lang->line('Error in the database while deleting the information');}
				}

			else {
				//---> There is at least one beekeeper or colony having this information
				$response['success'] = false;
				$response['messages'] = $this->lang->line('At least one beekeeper/association/apiary uses this information.  You cannot delete.');}

		}
		else {
			$response['success'] = false;
			$response['messages'] = $this->lang->line('Refresh the page again');}

		echo json_encode($response);
	}

}